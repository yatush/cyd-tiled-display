...
def generate_move_page_tile(config):
    ...
    modifiers = []
    if dynamic_entry:
        key = dynamic_entry.get("key", "")
        value = dynamic_entry.get("value", "")
        if key and value:
            modifiers.append(f'setDynamicEntry("{key}", {{ "{value}" }})')
        else:
            raise ValueError(f"dynamic_entry at ({x}, {y}) must have both 'key' and 'value'")
...
def generate_toggle_entity_tile(config):
    ...
    if not identifier:
        raise ValueError(f"toggle_entity tile at ({x}, {y}) must have 'identifier' field")
    if not entity:
    ...
    tile_cpp = f'new ToggleEntityTile({x}, {y}, {display_cpp}, "{identifier}", "{entity}", "{presentation_name}", {initially_chosen_cpp})'
    tile_cpp = _apply_modifiers(tile_cpp, config)
    tile_cpp += ','
    return tile_cpp
...
def generate_cycle_entity_tile(config):
    ...
    for option_item in options:
        if isinstance(option_item, dict):
            ...
            if value and label:
                options_cpp_pairs.append(f'{{ "{value}", "{label}" }}')
            else:
                raise ValueError(f"Each option item must have both 'value' and 'label' fields at ({x}, {y})")
        else:
            raise ValueError(f"Options must be dicts with 'value' and 'label' fields at ({x}, {y})")
